    ...
    def camera_callback(self, img_data):
        ...
        for area_cnt in colour_cnt_arr:
            ...
            if(area_cnt[0] >= 50):
                ...
                approx_distance2 = approx_dist_using_top(y, h)
                average_x = (x+x1)/2
                #calc x pixel position of middle of object
                middle = x+w/2

                #UNCOMMENT BELOW AFTER IMPLEMENTING approx_dist_using_top() METHOD
                # error = approx_dist - approx_distance2
                # threshold = 0.25 #in metres
                # if((error > threshold) or (error < threshold)):
                #     continue
...
#calculates the estimated distance using the location of the top of the cylinder
def approx_dist_using_top(y, height):
    #set the base known relationship of distance to y position of the cylinder top
    distance = 0.15 #in metres
    y_position = 200 #in pixels
    height = 200
    ratio = y/y_position
    fin_distance = distance * ratio
